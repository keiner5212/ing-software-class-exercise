...
export class CategoriaDAO {
	...
	protected static async add(nombre_categoria: string) {
		...
		return await db.task(async (t: pgPromise.IDatabase<any>) => {
			...
			const res1 = await t.manyOrNone(CategoriaRepository.GET_BY_NAME, [nombre_categoria]);
			if (res1.length > 0) {
	...
	protected static async update(id_categoria: number, nombre_categoria: string) {
		...
		return await db.task(async (t: pgPromise.IDatabase<any>) => {
			//check if exists
			const res1 = await t.oneOrNone(CategoriaRepository.GET_BY_ID, [id_categoria]);
			if (!res1) {
				throw new Error("request_err: there is no category with that id");
			}
			//check if exists already a category with that name
			const res2 = await t.manyOrNone(CategoriaRepository.GET_BY_NAME, [nombre_categoria]);
			if (res2 && res2.length > 0 && res2.every((obj) => obj.id_categoria !== id_categoria)) {
				throw new Error("request_err: there is already a category with that name");
			}
	...
	protected static async delete(id_categoria: number) {
		return await db.task(async (t: pgPromise.IDatabase<any>) => {
			...
			const res2 = await t.manyOrNone(CategoriaRepository.GET_MOVIES_BY_CATEGORY, [id_categoria]);
			if (res2 && res2.length > 0) {
				throw new Error("request_err: there are movies in that category");
			}
